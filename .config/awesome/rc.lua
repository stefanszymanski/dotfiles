...
run_once({ "$HOME/.scripts/lockscreen_auto.sh" })
...
local chosen_theme = "one"
local modkey       = "Mod4"
local altkey       = "Mod1"
local terminal     = "urxvtc"
local editor       = os.getenv("EDITOR") or "vim"
local browser      = "firefox"
local scrlocker    = "$HOME/.scripts/lockscreen.sh"
...
screenshot.defaults.actions_factory = function(path, args)
    return {
        ranger = function()
            path = path:gsub("~", "$HOME")
            awful.spawn.with_shell(terminal .. " -e ranger --selectfile=" .. path)
        end,
        edit = function()
            awful.spawn.with_shell("pinta " .. path)
        end,
    }
end
